...
export function WelcomeCard({ company, locale }: WelcomeCardProps) {
  ...
  return (
    <div className="animate-fade-up space-y-6">
      <div>
        <h1 className="font-sora text-3xl font-semibold text-[var(--text-heading)]">
      ...
      <div className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h2 className="font-sora font-semibold text-[var(--text-heading)] mb-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] p-5">
          ...
          <p className="font-sora font-semibold text-[var(--text-heading)] text-sm leading-snug">{companyName}</p>
        </div>
        <div className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] p-5">
          <p className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">
            {fr ? "Type de constitution" : "Incorporation type"}
          </p>
          <p className="font-sora font-semibold text-[var(--text-heading)] text-sm">{typeLabel}</p>
        </div>
        <div className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] p-5">
          <p className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="font-sora font-semibold text-[var(--text-heading)] text-sm">{province}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] p-6">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 bg-[var(--amber-400)] rounded-lg flex items-center justify-center">
            <svg className="w-4 h-4 text-[#1C1A17]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <span className="font-sora font-semibold text-[var(--text-heading)]">Sprint 2 {fr ? "arrive bientôt" : "coming soon"}</span>
        </div>
        <p className="text-sm text-[var(--text-muted)]">
          {fr
